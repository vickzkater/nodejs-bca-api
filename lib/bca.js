...
class BCA {
  ...
  async request({
    ...
    // const token = await this.getExpiredToken(); 
    // const signature = 'd22edc33f02b9715466ed8c1b56089a16e7f638c842a4e7db9affe8fab6ec221';
  ...
  /**
   * Make a request to BCA API - special for domestic transfer (by Vickzkater)
   * @param {String} param.method - Request method (POST, GET, PUT)
   * @param {String} param.relativeUrl - Request relative url
   * @param {String} param.body - Stringified Request body
   * @param {String} param.channelId - Channel Identification Number (Ex: 95051 for KlikBCA Bisnis)
   * @param {String} param.corpId - Your Channel Identity (ex: Your KlikBCA Bisnis CorporateID)
   * @return {Promise<Object,Error>} - Promised resolved with response json data
   */
  async vrequest({
    method = 'POST',
  ...
  async getSignature({
    ...
    // cek apakah ada koma (,) di dalam URL - function getBalance for >1 accounts
    if(relativeUrl.split(",").length > 1){
      var xsignature = hmac.update(`${method}:${relativeUrl.split(",").join("%2C")}:${accessToken}:${digestedBody}:${timestamp}`);
    }else{
      var xsignature = hmac.update(`${method}:${encodeURI(relativeUrl)}:${accessToken}:${digestedBody}:${timestamp}`);
    }
