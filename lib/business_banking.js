...
class BusinessBanking extends BCA {
  ...
  /**
   * Corporate transfer within domestic bank except BCA
   * @param {Object} body - request body
   * @param {String} corporateId - Channel Identity (ex: Your KlikBCA Bisnis CorporateID)
   * @param {String} channelId - Channel Identification Number (Ex: 95051 for KlikBCA Bisnis)
   * @return {Promise} Promise resolved with transaction is successfully sent or not
   */
  async domesticTransfer(body, corporateId, channelId) {
    const payload = {
      ...body,
      TransactionDate: format(new Date(), 'YYYY-MM-DD'),
    };
    const response = await this.vrequest({
      method: 'POST',
      body: JSON.stringify(payload),
